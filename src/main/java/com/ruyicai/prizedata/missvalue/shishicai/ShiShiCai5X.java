...
/**
 * 时时彩五星(选号栏)(1,2,3,4,5星)
 * @author liuhongxing
 *
 */
public class ShiShiCai5X{

	public static final String KEY = "T01007MV_5X";
	...
	public ShiShiCai5X refresh(PrizeInfoService prizeInfoService,int index) {
		...
		while(flag){
			...
			for(PrizeInfo p:prizeInfos) {
				if(this.refresh5X(p.getWinbasecode(), map, i)){
					i++;
				}else{
					flag = false;
					break;
	...
	private boolean refresh5X(String win,Map<String, Object> map,int i) {
		int[] winInt = { Integer.parseInt(win.charAt(0) + ""),
				Integer.parseInt(win.charAt(1) + ""),
				Integer.parseInt(win.charAt(2) + ""),
				Integer.parseInt(win.charAt(3) + ""),
	...
	public static ShiShiCai5X fromJsonToShiShiCai5X(String json) {
        return new JSONDeserializer<ShiShiCai5X>().use(null, ShiShiCai5X.class).deserialize(json);
    }
