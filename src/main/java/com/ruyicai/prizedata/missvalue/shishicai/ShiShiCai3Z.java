 ...
 *
 */
public class ShiShiCai3Z {

	public static final String KEY = "T01007MV_3Z";
	...
	public ShiShiCai3Z refresh(PrizeInfoService prizeInfoService,int index) {
		...
		while(flag){
			...
			for(PrizeInfo p:prizeInfos) {
				if(this.refresh6Z(p.getWinbasecode(), mark, i)){
					i++;
				}else{
					flag = false;
					break;
	...
	private boolean refresh6Z(String win,Map<String,Object> mark,int i) {
		int bai = Integer.parseInt(win.substring(2,3));
		int shi = Integer.parseInt(win.substring(3,4));
		int ge = Integer.parseInt(win.substring(4,5));
	...
	private boolean isZusan(int a,int b,int c) {
		int i = 0;
		if(a==b) {
			i++;
		}
	...
	public static ShiShiCai3Z fromJsonToShiShiCai6Z(String json) {
		return new JSONDeserializer<ShiShiCai3Z>().use(null, ShiShiCai3Z.class)
							.deserialize(json);
    }
