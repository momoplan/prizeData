 ...
 *
 */
public class Chongqing11c5R7ZH {

	public static final String KEY = "T01016MV_R7ZH";
	...
	public Chongqing11c5R7ZH refresh(PrizeInfoService prizeInfoService,int index) {
		...
		while(flag){
			...
			for(PrizeInfo p:prizeInfos) {
				if(this.refreshR7ZH(p.getWinbasecode(), map, i)){
					i++;
				}else{
					flag = false;
					break;
	...
	private boolean refreshR7ZH(String win,Map<String, Object> mark,int i) {
		String wan = win.substring(0,2);
		String qian = win.substring(2,4);
		String bai = win.substring(4,6);
		String shi = win.substring(6,8);
	...
	private Map<String, Object> initMap(Map<String, Object> map) {
		...
		BaseMath.combine(a, 11, 7, new int[7], 7, list);
	...
	public static Chongqing11c5R7ZH fromJsonToChongqing11c5R7ZH(String json) {
		return new JSONDeserializer<Chongqing11c5R7ZH>().use(null,
				Chongqing11c5R7ZH.class).deserialize(json);
	}
